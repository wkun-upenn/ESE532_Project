...
void sha256_final(SHA256_CTX *ctx, BYTE hash[]) {
    ...
    // Append the total message's length in bits
    ...
    // Copy the final state to the output hash
    for (i = 0; i < 4; ++i) {
...
// Helper function to compute SHA-256 hash over a specific range
void sha256_hash_range(const BYTE data[], uint32_t start_idx, uint32_t end_idx, BYTE hash_output[]) {
    SHA256_CTX ctx;
    sha256_init(&ctx);
    sha256_update(&ctx, data + start_idx, end_idx - start_idx);
    sha256_final(&ctx, hash_output);
}
...
// Helper function to compute SHA-256 hash and return as hexadecimal string
std::string sha256_hash_string(const BYTE data[], size_t length) {
    BYTE hash[SHA256_BLOCK_SIZE];
    sha256_hash(data, length, hash);
    return bytes_to_hex_string(hash, SHA256_BLOCK_SIZE);
}
